        ...
        for thing in ''.join(map(chr,map(ord,str(m)))).split('\n'):
            if len(thing) > 5:
                ...
                if len(minIONports) > 0:
                    ...
                    print (minIONports)
                    port = minIONports[0]
                    grpc_port = minIONports[1]
                    grpc_web_port = minIONports[2]
                    grpc_web_insecure_port = minIONports[3]
                    minIONdict[thing[1:8]]["port"]=port
                    minIONdict[thing[1:8]]["ws_longpoll_port"]=0
                    minIONdict[thing[1:8]]["grpc_port"]=grpc_port
                    minIONdict[thing[1:8]]["grpc_web_port"]=grpc_web_port
                    #minIONdict[thing[1:8]]["grpc_connection"] = rpc.Connection(
                    #    port=minIONdict[thing[1:8]]["grpc_port"],host=args.ip)
                    #minIONdict[thing[1:8]]["device_connection"] = DeviceConnect(minIONdict[thing[1:8]]["grpc_connection"])
        #            #print "port:",port
...
class ThreadingExample(object):
    ...
    def __init__(self, interval=3):
        ...
        if args.nolights is False: self.font3 = ImageFont.load("rpi-rgb-led-matrix/fonts/9x18B.pil")
    ...
    def write_time2(self,color):
        if args.nolights is False:
            timestring=time.strftime("%H:%M:%S", time.gmtime())
            messagelen=len(timestring)
            messagewindow = (messagelen * 10) + 50
            image = Image.new("1",(640,messagewindow))
            image = image.convert("RGBA")
            draw = ImageDraw.Draw(image)
            draw.text((0,-2),timestring,font=self.font3,fill=color)
            #self.matrix.Clear()
            while 1:
                for x in range (0,messagewindow):
                    self.matrix.SetImage(image.im.id,(50-x),0)
                    time.sleep(0.02)
                break
            #time.sleep(showtime)
            #self.matrix.Clear()

    def custom_write(self,message,color):
        if args.nolights is False:
	    #message length is
            messagelen=len(message)
            messagewindow = (messagelen * 10) + 50
            print messagelen
            image = Image.new("1",(messagewindow,messagewindow))
            image = image.convert("RGBA")
            draw = ImageDraw.Draw(image)
            draw.text((0,-2),message,font=self.font3,fill=color)
            self.matrix.Clear()
            while 1:
		for x in range (0,messagewindow):
         	    self.matrix.SetImage(image.im.id,(50-x),0)
            ...
            for minION in minIONdict:
                ...
                if minIONdict[minION]["state"]=="active":
                    ...
                    if "connected" not in minIONclassdict[minION]:
                        ...
                        minIONdict[minION]["grpc_connection"] = rpc.Connection(
                            port=minIONdict[minION]["grpc_port"], host=args.ip)
                        minIONclassdict[minION]["class"] = DummyClient(connectip) #,minIONdict[minION]["grpc_connection"])
                        minIONclassdict[minION]["device_connection"] = DeviceConnect(
                            minIONdict[minION]["grpc_connection"])
                    try:
                    #if 1:
                        if "connected" not in minIONclassdict[minION]:
                            try:
                                ...
                                example.custom_write("Extracting relevant datasets","blue")
